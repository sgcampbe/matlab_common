...
classdef dict < handle
    ...
    methods 
        ...
        function val = subsref(obj,ref)
            ...
            % STUMOD - enabling deeper subreferencing
            numrefs = length(ref);
            for i = 2:numrefs
                type = ref(i).type;
                subs = ref(i).subs;
                if strcmp(type, '()')    % Must be a numerical ref or cell array subref
                    val = val(subs);
                elseif strcmp(type, '{}')% Must be a cell array deref
                    val = val{subs};
                elseif strcmp(type, '.') % Must be accessing a property or struct field
                    val = val.(subs);
                else
                    error('ERROR: cannot parse subreference or unsupported subreference type!');
                end
            end
