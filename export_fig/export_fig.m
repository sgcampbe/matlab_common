...
if isvector(formats)
    ...
    if formats.eps
        try
            ...
            disp('Using Stuart mod to embed font, see export_fig.m (local copy, ~ line 207)')
            system(sprintf('/usr/local/bin/gs -sDEVICE=pswrite -dNOCACHE -sOutputFile=%s.eps -q -dbatch -dNOPAUSE %s -c quit', name, pdf_nam)) % SGC_mod: using ghost script directly to embed fonts
